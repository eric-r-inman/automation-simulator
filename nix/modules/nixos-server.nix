# NixOS (Linux/systemd) module for the automation-simulator-server service.
...
#
#   inputs.automation-simulator.nixosModules.server
#
...
#
#   locations."/".proxyPass =
#     "http://unix:${config.services.automation-simulator-server.socket}";
#
...
in {
  options.services.automation-simulator-server = {
    ...
    socket = lib.mkOption {
      ...
      default = "/run/automation-simulator-server/automation-simulator-server.sock";
      description = ''
    ...
    user = lib.mkOption {
      type = lib.types.str;
      default = "automation-simulator-server";
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.sockets.automation-simulator-server = lib.mkIf (cfg.socket != null) {
      description = "automation-simulator-server Unix domain socket";
    ...
    systemd.services.automation-simulator-server = {
      description = "automation-simulator-server service";
      wantedBy = ["multi-user.target"];
      after =
        ["network.target"]
        ++ lib.optional (cfg.socket != null) "automation-simulator-server.socket";
      requires =
        lib.optional (cfg.socket != null) "automation-simulator-server.socket";
      ...
      serviceConfig = {
        ...
        # Override via systemd.services.automation-simulator-server.serviceConfig.WatchdogSec.
